...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    config.loader.ensureEndpoint(located.endpoint, options.inject);
  ...
  .then(function() {
    ...
    .then(function() {
      ...
      // Check if an endpoint for the resolved package is defined, if not skip download
      if (resolution.endpoint === undefined)
        return Promise.resolve();
...
function logInstall(name, target, resolution, options) {
  ...
  else if (!resolution.endpoint) {
    verb = 'Stalled';
    version = resolution.exactName;
  }
  ...
  if (options.parent)
    ui.log('ok', verb + ' `' + target.exactName + '` (' + version + ')');
  else
    ui.log('ok', verb + ' %' + name + '% as `' + target.exactName + '` (' + version + ')');
}
      ...
      .then(function() {
        ...
        return secondaryDepsPromises[parent] = new Promise(function(resolve, reject) {
          ...
          fs.exists(pjsonPath, function(exists) {
            if (exists)
              return resolve();
            pkg.download(parentPkg, null, resolve).then(resolve, reject);
          });
        })
        .then(function(depMap) {
          if (depMap)
...
exports.resolveOnly = function(pkg) {
  ...
  if (!pkg.version || !pkg.endpoint) {
    ui.log('warn', 'Resolve --only must take an exact package of the form `endpoint:pkg@version`.');
