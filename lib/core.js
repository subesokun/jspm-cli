...
// if so, it does
// supports custom transpilerNames -> 'babel', 'traceur' (default)
  ...
  .then(function(cacheVersions) {
    ...
    .then(function(files) {
      ...
      files.forEach(function(file) {
        // NB deprecate 6to5 with 0.13
        if (file.match(/^system|es6-module-loader|traceur|babel|6to5/))
          found++;
      });
      if (found < 3)
        return exports.dlLoader(transpilerName);
    ...
    return Promise.all([
      ...
      babel ? dl('babel', 'npm:babel-core', !edge ? lVersions.babel : 'latest').then(function(version) {
        ...
        return Promise.all([
          ...
          cp('babel/external-helpers.js', 'babel-helpers.js'),
          cp('babel/browser-polyfill.js', 'babel-polyfill.js')
        ]);
